...
|---|------------|-------|----------|--------|
| **1** | **Mandatory adversarial review on stories** — every story that isn't pure "define constants with architect filter" gets adversarial review before `ready-for-dev` | SM | High | **Done** — story-review workflow created |
| **2** | **Architect critical-only filter** — after adversarial review, architect reviews findings and applies only critical ones | Architect | High | **Done** — step-05b-architect-filter.md added to quick-dev workflow |
| **3** | **Parallelism planning at epic/story creation** — embed dependency mapping and parallel batch identification when breaking epics into stories, not just at sprint-planning time | SM + Architect | High | **Done** — step 4b added to create-story/instructions.xml + Parallelism section added to template.md |
...
| **5** | **Agent team pipeline with worktrees** — design step file for automated: create-story → adversarial review → architect filter → dev → typecheck → code review → E2E (real UI) + fix → PR, each in isolated worktree branches using Claude Code agent teams | SM + Dev | High | **Done** — agent-team-pipeline/workflow.md created with 8-stage pipeline spec |
| **6** | **TypeScript errors caught too late** — (a) add typecheck to pre-commit hook, (b) dev/review/fix agents must all run typecheck as mandatory gate at every stage | Dev | High | **(a) Done** — typecheck added to pre-commit hook + turbo task + all packages; **(b) Done** — typecheck mandatory gate added to step-03 (dev), step-04 (self-check), step-06 (fix) |
| **7** | **Mandatory E2E step per story** — Playwright E2E as final gate before PR; tests MUST use actual UI interactions (clicks, forms, navigation), NOT direct API calls; agent MUST extend existing test files and user paths before creating new ones; validates UX from frontend perspective | QA + Dev | High | **Done** — step-07-e2e-validation.md added as final gate in quick-dev workflow |
...
| **11** | **Document placeholder-row pattern** — formalize as architectural pattern in ARCHITECTURE.md (appeared in 3+ stories) | Architect | Low | **Done** — expanded in ARCHITECTURE.md with 4-part lifecycle, ownership rules, and use-case mapping |
