...
editHistory:
  - date: '2026-04-30'
    changes: 'Grill-with-docs Q12-C: FR10/FR49 — MVP at most one `activate-conversation-extension` per account; second blocked regardless of Polar cancelled_active / letter tier; FR101b does not waive cap. UX journey “Multiple extensions” row; architecture extension use-case + ADR-47 pseudocode; CONTEXT.'
  - date: '2026-04-30'
    changes: 'Grill-with-docs Q11-C: FR101b — Nerin-voiced lapse acknowledgment in ExtensionContinueModule (no pricing/checkout in-module).'
  - date: '2026-04-30'
    changes: 'Grill-with-docs Q10-B: new FR101b (subscription lapse mid-extension grace), FR23/FR49, architecture ADR-47 note, UX Journey 4 error row, CONTEXT.'
  - date: '2026-04-30'
    changes: 'Grill-with-docs Q9-B: `ExtensionContinueModule` on **`/today`** and hub **`/me`** — **primary** `/chat` + **required secondary** text link to **`/me/$extensionSessionId`**; on **`/me/$extensionSessionId`** when id matches extension, **primary `/chat` only** (omit self-link). FR101a + UX §15.1.1 + CONTEXT.'
  - date: '2026-04-30'
    changes: 'Grill-with-docs Q8: hub `/me` — identity hero stays **latest finalized** assessment; `ExtensionContinueModule` alone foregrounds unfinalized extension (copy = open Nerin thread, not second hero). FR101a (2) + UX Me hub + CONTEXT.'
  - date: '2026-04-30'
    changes: 'Grill-with-docs Q7: `ExtensionResumeHint` primary link is **`/me/$extensionSessionId`** (session-scoped Me for the extension), not `/chat` — chat resume stays on `ExtensionContinueModule` / extension Me. FR101a + CONTEXT updated.'
  - date: '2026-04-30'
    changes: 'Grill-with-docs Q6: FR101a session-scoped Me — full `ExtensionContinueModule` only when `/me/$conversationSessionId` matches the unfinalized extension session; other session-scoped Me pages show `ExtensionResumeHint` (one line + link; primary target refined in Q7).'
  - date: '2026-04-30'
    changes: 'Grill-with-docs: phase triggers (habit → Coach) unchanged from prior 2026-04-30 edit. Authenticated landing — initial assessment forces `/chat` until first Assessment Finalization; after that `/today` stays default; unfinalized subscriber extension surfaced via new FR101a (Today module + Me placement). FR101/FR102, executive summary, journey table, Innovation #10, capabilities row updated.'
  - date: '2026-04-18'
...
**Product shape: three-space architecture.** The authenticated product is organized around three spaces, not a dashboard: **Today** (ephemeral daily companion — silent journal check-in, mood calendar, weekly letter from Nerin), **Me** (persistent identity — portrait, archetype, scores, public face, subscription), and **Circle** (the few people you care about — relationship letters, invite ceremony). Assessment is an onboarding tunnel into this world, not a destination. Until the **first** **Assessment Finalization** (initial 15-exchange assessment complete), authenticated entry (e.g. app root after login) resolves to **`/chat`** so the user always resumes that in-progress conversation; after the one-time post-assessment sequence on **Me**, **`/today`** is the primary daily landing surface. A **subscriber extension** (+15 exchanges) that is paused does **not** change default entry — **`/today`** stays default; an explicit **continue-extension** affordance marks unfinished extension work (FR101a).
...
### Phase transition triggers (habit loop → Coach depth)

MVP subscription (€9.99/mo — conversation extension plus bundled first-extension portrait regeneration; see ADR-47) ships with the core product. **These five triggers do not gate Polar checkout or the MVP subscription SKUs.** They gate shipping **Coach agent** and other **Phase 2** subscription-depth layers once the free habit loop (silent journal, weekly letter, relationship letter, Circle) has shown retention.

The trigger to **launch Coach agent** is metric-based, not time-based. When 4 of 5 fire → proceed with Coach / Phase 2 depth:
...
- **Today** — primary daily landing **after** **Assessment Finalization** and the post-assessment **Me** sequence; silent daily journal (mood + optional note, no LLM response), mood calendar, inline weekly letter card on Sundays, contextually-surfaced library article (2–3/week)
...
**Capabilities revealed:** Three-space navigation: while the **initial** assessment awaits **Assessment Finalization**, authenticated entry resolves to **`/chat`**; after first finalization, `/today` is primary daily landing; **unfinalized subscriber extension** sessions use FR101a surface on Today/Me, not forced `/chat` entry (FR101, FR101a, FR102). Silent daily check-in with mood + note (FR67, FR68), weekly anticipation line (FR68a), mood calendar (FR70), three-level note visibility (FR71), free weekly summary LLM generation (FR86, FR87), push notification + focused reading route (FR89, FR90), soft conversion CTA (FR91), scheduled daily prompt per personality profile (FR96), return seed → weekly letter bridge.
...
The authenticated product is organized around three spaces, not a dashboard: **Today** (ephemeral daily — silent journal, mood calendar, weekly letter inline card on Sundays), **Me** (persistent identity — portrait, archetype, radar, public face, subscription, Circle preview), and **Circle** (the few people you care about — relationship letters, invite ceremony). `/dashboard` is removed. Until the **first** **Assessment Finalization**, authenticated entry resolves to **`/chat`** (resume the initial assessment). The post-assessment sequence is **focused portrait reading** at **`/me/$conversationSessionId?view=portrait`**, then the **session-scoped Me surface** at **`/me/$conversationSessionId`** (FR93–FR95); legacy **`/results/*`** URLs may exist only as redirects. After that, the user navigates freely across Today/Me/Circle without a persistent first-visit route gate. `/today` remains the primary daily landing surface; **paused subscriber extensions** stay visible via FR101a without stealing default entry from **`/today`** (they do not force **`/chat`** on app open). Assessment (`/chat`) sits outside the three-space world as an onboarding tunnel. The architectural bet: linear "home → chat → Me → done" is a dead end that creates post-assessment churn; a hub-and-spoke model with distinct return reasons creates a habit loop. This is what turns a one-time personality test into a daily companion.
...
- **FR10:** *(Subscription — MVP)* Subscribers can extend their conversation (+15 exchanges) to continue with Nerin. Extension is the sole paid perk in MVP (alongside bundled first-extension portrait regeneration per FR23). **MVP cap (grill Q12-C):** At most **one** successful **`activate-conversation-extension`** per subscriber account in MVP — a **second** activation is **out of scope** even if Polar still shows an active billing period (e.g. cancelled-but-period-not-ended); **FR101b** covers **finishing** an extension session already opened, not a second activation. *Acceptance: subscribed user can trigger **one** conversation extension (per FR49); extension creates a new conversation session initialized with the prior session's final state (FR25)*
...
- **FR23:** *(Subscription — MVP)* The first conversation extension per subscriber automatically generates a new portrait at no additional cost beyond the €9.99/mo subscription. The new portrait incorporates observations derived from the extended evidence not present in the original. The prior portrait remains attached to the prior assessment result as "previous version" on the Me page. *Acceptance: a user who completes their first conversation extension sees a new portrait generated automatically without any additional purchase step (including when subscription lapses **after** the extension began but **before** **Assessment Finalization** — **FR101b**); the original portrait remains visible as "previous version"*
...
- **FR49:** *(Subscription — MVP)* Subscribers have access to conversation extensions (FR10) as part of their subscription. **Starting** a **new** `activate-conversation-extension` flow requires **active** paid entitlement **and** the MVP **one-extension** cap (FR10 — at most **one** successful activation per account in MVP). **Mid-flight grace (FR101b):** an extension session **already created** while subscribed may run to **Assessment Finalization** even if `subscription_cancelled` / `subscription_expired` arrives before the last exchange. **Grill Q12-C:** A **second** `activate-conversation-extension` is **blocked** in MVP regardless of whether the user still appears “subscribed” until Polar period end (e.g. weekly letter behavior per **FR92(c)** may still apply independently — it does **not** grant a second extension activation). Post-MVP repeat extensions follow FR23a / Phase 2a
...
- **FR101:** For the user's **initial** 15-exchange **Authenticated Conversation**, until **Assessment Finalization** completes for that session, authenticated entry (`/` or equivalent “open app” targets) resolves to **`/chat`** to resume. After the closing exchange (FR12), the user enters the post-assessment transition (FR93–FR96): first the **focused portrait reading** at `/me/$conversationSessionId?view=portrait`, then the **full Me identity surface** at `/me/$conversationSessionId` (see FR95). That sequence is the emotional peak. After **first** **Assessment Finalization**, routine authenticated entry is **`/today`** (primary daily surface) even if a **subscriber extension** session is in progress; unfinished extensions are surfaced per **FR101a**. Thereafter users navigate freely across Today / Me / Circle; legacy `/results/*` URLs may remain as redirects only (UX addendum — canonical `/me` paths)
- **FR101a:** When a **subscriber extension** session (+15 exchanges) exists and has **not** reached **Assessment Finalization** for that session, the product exposes a **continue-extension** affordance so the state is never invisible. **`ExtensionContinueModule`** (full module) **MVP contract (grill Q9-B):** **primary** control → **`/chat`** for that extension session + **secondary** text link → **`/me/$extensionSessionId`** (session-scoped Me for the extension) on **`/today`** and on **Me hub `/me`** — both links are **required** there. On **session-scoped** **`/me/$conversationSessionId`** where the id **equals** the extension session id, render the same module with **primary `/chat` only** — **omit** the secondary link when it would merely deep-link to the **current** URL (no redundant self-link). (1) **`/today`:** module **above** the silent check-in form; (2) **hub `/me`:** module in subscription / continuation zone **without** swapping the **identity hero** (hero stays **latest finalized** assessment — grill Q8); copy frames the module as **open conversation with Nerin**, not a second identity row; (3) **other** session-scoped Me URLs → **`ExtensionResumeHint`** only — one Nerin-voiced line + **primary** link to **`/me/$extensionSessionId`**; **`/chat`** is not the hint’s primary target. User can still complete the daily check-in on **`/today`** first; no streak shame, no numeric “days left.” Surfaces stay until that extension’s **Assessment Finalization** completes
- **FR102:** The three-space bottom navigation (Today / Me / Circle) is the primary navigation model for authenticated users after the initial **Assessment Finalization**. `/dashboard` is removed. A thin `/settings` route contains account admin (email, password, data export, delete) accessed via a gear icon on the Me page. Assessment (`/chat`) sits outside the three-space world as an onboarding tunnel — users land in `/chat` from pre-conversation onboarding after signup + verification **and** on authenticated entry while the **initial** assessment awaits **Assessment Finalization** (FR101). **Subscriber extensions** do not change default entry after first finalization (FR101a)
- **FR101b:** *(Subscription lapse — grill Q10-B)* If **`subscription_cancelled`** or **`subscription_expired`** (Polar-derived, ADR-47) occurs while a **subscriber extension** `assessment_session` exists but has **not** completed **Assessment Finalization**, the user **may still** send messages in **`/chat`**, complete the extension, run **Assessment Finalization**, and receive outcomes already promised for that extension (including bundled first-extension portrait per **FR23** when applicable). **`activate-conversation-extension`** (creating a **new** extension session) remains gated on **active** paid entitlement **and** the MVP one-extension cap (**FR49**, grill **Q12-C** — not waived by lapse). **`ExtensionContinueModule` / `ExtensionResumeHint`** (FR101a) stay until that session finalizes; copy must **not** block or paywall finishing an already-open thread. **Grill Q11-C:** When this lapse state applies, **`ExtensionContinueModule`** adds **one** extra **Nerin-voiced** acknowledgment that the **paid subscription** relationship has ended while **this thread** can still be finished (example tone: *“We can still finish what we started — that stays between us.”* — final copy UX-owned); **no** € amounts, **no** merchant/SKU strings, **no** checkout or resubscribe **CTA inside this module** (renewal paths remain Me subscription area, weekly letter, **`/settings`**, etc.). **`ExtensionResumeHint`** may fold the same acknowledgment into its **single** Nerin line when lapse applies — still **no** pricing there either
