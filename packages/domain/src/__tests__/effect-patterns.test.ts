...
describe("Effect-ts Service Testing", () => {
  ...
  describe("Database Mock Testing", () => {
    ...
    it("should find sessions by user ID", async () => {
      ...
      const user123Sessions = await Effect.runPromise(
        db.sessions.findByUserId("user_123"),
      );
  ...
  describe("Cost Guard Testing", () => {
    it("should track token usage and update budget", async () => {
      ...
      const initialBudget = await Effect.runPromise(
        costGuard.getRemainingBudget(),
      );
      expect(initialBudget).toBe(75.0);

      await Effect.runPromise(
        costGuard.trackUsage({ input: 1000, output: 500 }),
      );

      const remainingBudget = await Effect.runPromise(
        costGuard.getRemainingBudget(),
      );
