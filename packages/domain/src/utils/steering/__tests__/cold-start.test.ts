/**
 * Cold-Start Territory Selection Tests — Story 21-4, evolved Story 23-2
 *
 ...
 *
 * Note: COLD_START_TERRITORIES constant was removed in Story 23-2.
 * Cold-start selection now uses caller-provided territory lists.
 */
...
/** Test cold-start territory IDs */
const TEST_COLD_START_TERRITORIES: readonly TerritoryId[] = [
...
describe("selectColdStartTerritory", () => {
	it("returns first cold-start territory for message index 0", () => {
		const result = selectColdStartTerritory(0, TEST_COLD_START_TERRITORIES);
		expect(result.territoryId).toBe(TEST_COLD_START_TERRITORIES[0]);
	});

	it("returns second cold-start territory for message index 1", () => {
		const result = selectColdStartTerritory(1, TEST_COLD_START_TERRITORIES);
		expect(result.territoryId).toBe(TEST_COLD_START_TERRITORIES[1]);
	});

	it("returns third cold-start territory for message index 2", () => {
		const result = selectColdStartTerritory(2, TEST_COLD_START_TERRITORIES);
		expect(result.territoryId).toBe(TEST_COLD_START_TERRITORIES[2]);
	});

	it("wraps around with round-robin for index >= array length", () => {
		const result = selectColdStartTerritory(3, TEST_COLD_START_TERRITORIES);
		expect(result.territoryId).toBe(TEST_COLD_START_TERRITORIES[0]);
	});
...
describe("selectTerritoryWithColdStart", () => {
	...
	it("uses cold-start for message 0", () => {
		const result = selectTerritoryWithColdStart({
			...
			coldStartTerritories: TEST_COLD_START_TERRITORIES,
			scoredTerritories,
		});
		expect(result.territoryId).toBe(TEST_COLD_START_TERRITORIES[0]);
	});
	...
	it("respects configurable threshold (threshold=5)", () => {
		...
		expect(result.territoryId).toBe(TEST_COLD_START_TERRITORIES[1]); // 4 % 3 = 1
	});
