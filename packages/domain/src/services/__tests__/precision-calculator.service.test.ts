...
describe("Precision Calculator Service", () => {
  describe("calculateTraitPrecision", () => {
    it("should calculate trait precision from facet scores", () => {
      ...
      const result = calculateTraitPrecision(facetPrecision);
  ...
  describe("initializeFacetPrecision", () => {
    it("should initialize all facets with default baseline", () => {
      const result = initializeFacetPrecision();
    ...
    it("should initialize all facets with custom baseline", () => {
      const result = initializeFacetPrecision(0.7);
    ...
    it("should initialize with extreme baseline values", () => {
      const resultHigh = initializeFacetPrecision(1.0);
      Object.values(resultHigh).forEach((value) => {
        expect(value).toBe(1.0);
      });

      const resultLow = initializeFacetPrecision(0.0);
  ...
  describe("updateFacetPrecision", () => {
    it("should update a single facet precision", () => {
      const initial = initializeFacetPrecision(0.5);
      const updated = updateFacetPrecision(initial, "imagination", 0.8);
    ...
    it("should clamp values to [0, 1]", () => {
      ...
      const tooHigh = updateFacetPrecision(initial, "imagination", 1.5);
      expect(tooHigh.imagination).toBe(1.0);

      const tooLow = updateFacetPrecision(initial, "imagination", -0.5);
      expect(tooLow.imagination).toBe(0.0);
    });

    it("should preserve all other facets", () => {
      const initial = initializeFacetPrecision(0.3);
      const updated = updateFacetPrecision(initial, "anxiety", 0.9);
  ...
  describe("mergePrecisionScores", () => {
    it("should merge with default 50/50 weight", () => {
      const current = initializeFacetPrecision(0.2);
      const update: Partial<FacetPrecisionScores> = {
        imagination: 0.8,
        artistic_interests: 0.8,
      };

      const result = mergePrecisionScores(current, update);
    ...
    it("should apply custom weight correctly", () => {
      const current = initializeFacetPrecision(0.0);
      const update: Partial<FacetPrecisionScores> = {
        imagination: 1.0,
      };

      const result = mergePrecisionScores(current, update, 0.75);
    ...
    it("should clamp weight to [0, 1]", () => {
      ...
      const resultHigh = mergePrecisionScores(current, update, 1.5);
      expect(resultHigh.imagination).toBe(1.0);

      // Weight < 0 should be clamped to 0
      const resultLow = mergePrecisionScores(current, update, -0.5);
      expect(resultLow.imagination).toBe(0.0);
    });

    it("should only update facets in the update object", () => {
      const current = initializeFacetPrecision(0.5);
      const update: Partial<FacetPrecisionScores> = {
        imagination: 0.9,
      };

      const result = mergePrecisionScores(current, update, 0.5);
    ...
    it("should handle weight of 0 (no update)", () => {
      const current = initializeFacetPrecision(0.3);
      const update: Partial<FacetPrecisionScores> = {
        imagination: 0.9,
      };

      const result = mergePrecisionScores(current, update, 0);
    ...
    it("should handle weight of 1 (full update)", () => {
      const current = initializeFacetPrecision(0.1);
      const update: Partial<FacetPrecisionScores> = {
        imagination: 0.9,
      };

      const result = mergePrecisionScores(current, update, 1);
