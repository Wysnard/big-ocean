 ...
 */
export const mockAnthropicResponse = (content: string, usage = { input_tokens: 100, output_tokens: 50 }) => ({
...
export const mockAnalyzer = (messages: string[]): Effect.Effect<AnalysisResult, never, never> =>
  Effect.succeed({
...
export const mockScorer = (
  messages: string[],
): Effect.Effect<TestSession["precision"], never, never> =>
  Effect.succeed({
...
export const mockRateLimiter = (): RateLimiter => {
  ...
  return {
    checkLimit: (key) =>
      Effect.succeed((requests.get(key) || 0) < 100), // Max 100 requests per key
    recordRequest: (key) =>
      Effect.sync(() => {
