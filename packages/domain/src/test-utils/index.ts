 ...
 */
export const mockNerin = (
  userMessage: string,
): Effect.Effect<string, never, never> =>
  Effect.succeed(
    `Thank you for sharing "${userMessage}". How does that make you feel?`,
  );
 ...
 */
export const mockScorer = (): Effect.Effect<
  TestSession["precision"],
  never,
  never
> =>
  Effect.succeed({
...
export const mockDatabase = () => {
  ...
  return {
    sessions: {
      insert: (session: TestSession) =>
        Effect.sync(() => sessions.set(session.id, session)),
