/**
 * Conversanalyzer Anthropic Repository Implementation (v2 + v3 split)
 *
 * v2: Single Haiku call for dual extraction (userState + evidence).
 * v3: Split into two separate Haiku calls — user state + evidence independently.
 *
...
// ─── v3 Split Prompts (Story 42-2) ────────────────────────────────────────────

/** @internal Exported for testing — user state extraction only (Phase 1) */
export function buildUserStatePrompt(input: ConversanalyzerInput): string {
	const recentText = input.recentMessages.map((m) => `[${m.role}]: ${m.content}`).join("\n");

	return `You are a conversational state extractor. Analyze the latest user message and classify the user's conversational state along two independent axes.
...
export function buildEvidencePrompt(input: ConversanalyzerInput): string {
	...
	return `You are a personality evidence extractor. Extract Big Five personality signals from the latest user message.
...
export const ConversanalyzerAnthropicRepositoryLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		// v3 split models — separate structured output schemas for user state and evidence
		...
		logger.info("Conversanalyzer v2+v3 configured", {
			model: config.conversanalyzerModelId,
		});
					...
					try: async () => {
						...
						try {
							...
							const formattedError = ParseResult.isParseError(parseError)
								? ParseResult.TreeFormatter.formatErrorSync(parseError)
								: String(parseError);
							logger.warn("ConversAnalyzer v3 user state strict decode failed", {
								error: formattedError,
								rawOutput: JSON.stringify(invokeResult.parsed).slice(0, 2000),
							});
							throw parseError;
						...
						logger.info("ConversAnalyzer v3 user state strict extraction complete", {
						...
						new ConversanalyzerError({
							message:
								error instanceof Error ? error.message : "ConversAnalyzer v3 user state strict failed",
						}),
				}),
			...
			analyzeUserStateLenient: (input: ConversanalyzerInput) =>
				Effect.tryPromise({
					try: async () => {
						...
						logger.info("ConversAnalyzer v3 user state lenient extraction complete", {
							energyBand: parsed.energyBand,
							tellingBand: parsed.tellingBand,
							tokenUsage,
						});
						...
						new ConversanalyzerError({
							message:
								error instanceof Error ? error.message : "ConversAnalyzer v3 user state lenient failed",
						}),
				}),
					...
					try: async () => {
						...
						try {
							...
							const formattedError = ParseResult.isParseError(parseError)
								? ParseResult.TreeFormatter.formatErrorSync(parseError)
								: String(parseError);
							logger.warn("ConversAnalyzer v3 evidence strict decode failed", {
								error: formattedError,
								rawOutput: JSON.stringify(invokeResult.parsed).slice(0, 2000),
							});
							throw parseError;
						...
						logger.info("ConversAnalyzer v3 evidence strict extraction complete", {
							evidenceCount: parsed.evidence.length,
							tokenUsage,
						});
						...
						new ConversanalyzerError({
							message:
								error instanceof Error ? error.message : "ConversAnalyzer v3 evidence strict failed",
						}),
				}),
					...
					try: async () => {
						...
						if (rawInput.evidence) {
							...
							for (let i = 0; i < rawInput.evidence.length; i++) {
								...
								const result = decodeItem(item);
								if (Either.isLeft(result)) {
									logger.warn("Invalid evidence item from ConversAnalyzer v3", {
										index: i,
										rawItem: JSON.stringify(item).slice(0, 500),
										error: ParseResult.TreeFormatter.formatErrorSync(
											new ParseResult.ParseError({ issue: result.left.issue }),
						...
						const discardedCount = rawCount - parsed.evidence.length;
						if (discardedCount > 0) {
							logger.warn("Discarded invalid evidence items (v3 lenient)", {
								discardedCount,
								rawCount,
								validCount: parsed.evidence.length,
							});
						...
						logger.info("ConversAnalyzer v3 evidence lenient extraction complete", {
							evidenceCount: parsed.evidence.length,
							discardedCount,
							tokenUsage,
						});
						...
						new ConversanalyzerError({
							message:
								error instanceof Error ? error.message : "ConversAnalyzer v3 evidence lenient failed",
						}),
				}),
		});
	}),
