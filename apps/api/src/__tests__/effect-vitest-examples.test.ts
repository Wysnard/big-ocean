	...
	describe("TestClock Examples", () => {
		it.effect("should advance virtual time with TestClock", () =>
			Effect.gen(function* () {
				...
				const deferred = yield* Effect.fork(Effect.sleep("5 seconds").pipe(Effect.as("completed")));
	...
	describe("Real-world Use Case Examples", () => {
		...
		it.effect("should test error recovery patterns", () =>
			Effect.gen(function* () {
				...
				const exit = yield* Effect.exit(sessionRepo.getSession("non-existent-session-id"));
