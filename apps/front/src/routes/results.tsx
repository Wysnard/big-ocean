...
const ResultsSearchParams = S.Struct({
  sessionId: S.optional(S.String),
  scrollToFacet: S.optional(S.String),
});

export const Route = createFileRoute("/results")({
  validateSearch: (search) => S.decodeUnknownSync(ResultsSearchParams)(search),
  beforeLoad: ({ search }) => {
    if (search.sessionId) {
      throw redirect({
...
function ResultsRouteShell() {
  ...
  const [entryState, setEntryState] = useState<"checking" | "none" | "expired">(
    "checking",
  );
      ...
      <div className="min-h-[calc(100dvh-3.5rem)] bg-background flex items-center justify-center px-6">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Checking for your pending results...
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-[calc(100dvh-3.5rem)] bg-background flex items-center justify-center px-6">
        <div className="w-full max-w-xl rounded-2xl border border-border bg-card p-6 text-center shadow-sm">
          <h1 className="font-heading text-2xl text-foreground">
            Your previous results unlock expired
          </h1>
          <p className="mt-2 text-muted-foreground">
            For privacy, gated result sessions are kept for 24 hours on this
            device.
          </p>
