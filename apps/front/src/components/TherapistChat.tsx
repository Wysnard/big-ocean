...
const MILESTONES = [
  {
    threshold: 25,
    message:
      "We're off to a great start — I'm already seeing some interesting patterns.",
  },
  {
    threshold: 50,
...
export function TherapistChat({
  ...
  const [shownMilestones, setShownMilestones] = useState<Map<number, number>>(
    new Map(),
  );
  ...
  useEffect(() => {
    for (const milestone of MILESTONES) {
      if (
        avgConfidence >= milestone.threshold &&
        !shownMilestones.has(milestone.threshold)
      ) {
  ...
  const inputPlaceholder = useMemo(() => {
    ...
    if (isConfidenceReady && !hasShownCelebration)
      return "Keep chatting to add more depth...";
    return placeholder;
  ...
  return (
    <>
      <ChatContent
        ...
        depthProgress={Math.min(
          userMessageCount / (freeTierMessageThreshold || 27),
          1,
        )}
          ...
          <div className="h-full flex flex-col mx-auto max-w-[900px] min-[1200px]:max-w-[1000px] min-[1440px]:max-w-[1100px] px-6">
                  ...
                  messages.map((msg, index) => (
                    ...
                    >
                      ...
                      {MILESTONES.map((milestone) =>
                        shownMilestones.get(milestone.threshold) ===
                        index + 1 ? (
                          <div
                  ...
                  >
                    <div className="bg-card border-2 border-primary rounded-2xl p-6 text-center shadow-lg">
                      ...
                      <div className="mt-4 flex gap-3 justify-center">
                        ...
                        <Button
                          variant="outline"
                          onClick={() => setHasShownCelebration(true)}
                        >
                          Keep Exploring
                        </Button>
                      </div>
            ...
            {errorMessage && (
              <ErrorBanner
                ...
                onRetry={
                  errorType === "network" || errorType === "generic"
                    ? retryLastMessage
                    : undefined
                }
                onDismiss={clearError}
                autoDismissMs={
                  errorType === "budget" || errorType === "rate-limit"
                    ? 0
                    : 5000
                }
              />
