...
function generateMockResponse(userMessage: string): string {
  ...
  if (
    ...
    return responses.adventure[
      Math.floor(Math.random() * responses.adventure.length)
    ];
  }
  if (
    ...
    return responses.challenge[
      Math.floor(Math.random() * responses.challenge.length)
    ];
  }
  if (
    ...
    return responses.creative[
      Math.floor(Math.random() * responses.creative.length)
    ];
  }
  if (
    ...
    return responses.relationship[
      Math.floor(Math.random() * responses.relationship.length)
    ];
  }
  if (
        ...
        sendMessageRpc(
          { sessionId, message: userMessage },
          {
            onSuccess: (data) => {
              ...
              setTimeout(
                () => {
                  ...
                  const mockResponse =
                    data.response || generateMockResponse(userMessage);
                  setMessages((prev) => [
                    ...prev,
                    {
            ...
            onError: (error) => {
              ...
              setTraits((prev) => {
                const newOpenness = Math.min(
                  prev.openness + Math.random() * 0.04,
                  0.95,
                );
                const newConscientiousness = Math.min(
                  prev.conscientiousness + Math.random() * 0.04,
                  0.95,
                );
                const newExtraversion = Math.min(
                  prev.extraversion + Math.random() * 0.04,
                  0.95,
                );
                const newAgreeableness = Math.min(
                  prev.agreeableness + Math.random() * 0.04,
                  0.95,
                );
                const newNeuroticism = Math.min(
                  prev.neuroticism + Math.random() * 0.04,
                  0.95,
                );
