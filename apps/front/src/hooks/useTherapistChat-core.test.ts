...
describe("useTherapistChat", () => {
	...
	it("adds user message optimistically before API responds", () => {
		...
		// 2 greeting messages + 1 user message
		expect(result.current.messages).toHaveLength(3);
		expect(result.current.messages[2].role).toBe("user");
		expect(result.current.messages[2].content).toBe("I love hiking");
	...
	it("adds assistant message on mutation success", () => {
		...
		// 2 greeting + user message + assistant response = 4
